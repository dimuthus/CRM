...
class ReportController extends Controller
{
    ...
    public function behaviors()
    {
        return [
            'access' => [
                ...
                'only' => ['search', 'complete', 'summary','outbound','csat','customer'],
                'rules' => [
                    [
                       'actions' => ['complete','outbound','customer'],
                       'allow' => true,
                       'roles' => ['@'],
    ...
    public function actionGenerateCustomer()
    {
        ...
        $params =Yii::$app->request->queryParams;
		$searchModel = new ReportSearch();
        $results = $searchModel->generateCustomer($params);
        return $this->renderAjax('customer_result', [ 'dataProvider' => $results, 'pagination' => false ]);
 ...
 public function actionGenerateSummaryOutbound()
	...
	public function actionActivitylog()
    {
        ...
        $dataProvider = new ActiveDataProvider([
            'query' => CsatSurvey::find()->where(['id' => -1])->groupBy('respondent_id')->orderBy('id DESC')
        ]);
	...
	public function logActivity($moduleName){
	  ...
	  \Yii::$app->db->createCommand("INSERT INTO audit_log (module_name,dml_timestamp,dml_created_by,dml_type,ip_address) VALUES ('$moduleName',NOW(),$dml_created_by,'VIEW','$ip')")->execute();
