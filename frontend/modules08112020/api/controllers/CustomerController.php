...
class CustomerController extends ActiveController
{
	...
	$mobile_number=\Yii::$app->request->post('cli');
	   ...
	   if ($mobile_number=="" AND $tpin=="" ){
		...
		$queryTpin=Yii::$app->db->createCommand("SELECT count(t_pin) as totT FROM customer WHERE t_pin=:t_pin AND mobile_number=:mobile_number ")->bindValue(":t_pin", $tpin)->bindValue(":mobile_number", $mobile_number);
	...
	public function actionUpdatelanguage(){
	   ...
	   if ($mobile_number=="" AND $pl=="" ){
		...
		$result = $queryCli->queryAll();
        $rowCountM=(int)$result[0]['totM'];  
		$queryPl=Yii::$app->db->createCommand("SELECT count(id) as totPl FROM customer_language WHERE id=:pl")->bindValue(":pl", $pl);
...
->andWhere(['t_pin' => \Yii::$app->request->post('tpin')]);		
	...
	public function validate($param){
		...
		if ($rowCount==0){
			if ($param=='cli'){
			$error=array('mobile_number'=>$request_param,'VIP'=>'NO');
			}
			else{
			$error=array('error'=>true,'message'=>'Invalid '.$param);
