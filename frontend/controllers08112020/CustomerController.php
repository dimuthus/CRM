...
class CustomerController extends Controller
{
    ...
    public function actionView($id,$jmc)
    {
      ...
      $logmodel->createViewLog("Customer",$data);
...
//        ]);
    ...
    { 
      $logmodel= new Logadm();
           $data = array('customer_id' => $id);
                  $data=json_encode($data);
      $logmodel->createViewLog("Customer:Details",$data);
    ...
    public function actionUpdate($id,$jmc)
    {
      ...
      $request_hash= $jmcIns->HashMe($getID);
        ...
        $oldData['mobile_number']=$model->decryptString($oldData['mobile_number']);
        $oldData['full_name']=$model->decryptString($oldData['full_name']);
        $oldData['passport']=$model->decryptString($oldData['full_name']);
		    $oldData['email']=$model->decryptString($oldData['email']);
...
//uploader code ends here....
    ...
    public function actionGetcustomerybymrn($mrn) {
        ...
        if (count($result) > 0) {
			...
			$this->call_log($mrn,$customerId,$CallId);
            return $this->redirect(['view', 'id' => $model->id, 'jmc' => $hashID,'CallId'=>1234567]);
         } 
		 else {
			 // $this->call_log($mrn,0,$CallId);
			 //die('something wrogm');
              return $this->redirect('create', ['model' => $model, 'jmc' => $hashID,'CallId'=>1234567]);
         }
	...
	protected function call_log($mrn,$customer_id, $callId) {
	...
	protected function findModelTest($id) {
	  ...
	  public function actionCampaginDataUpload(){
        ...
        $logmodel= new Logadm();
            // var_dump($model);
            // die();
              $logmodel->createUpdateLogRule("FileUploadCampaign: ","",$fileName,"INSERT");
  ...
  public function actionDailyCustomerUpdate(){
      ...
      $fileName="";
	    ...
	    //  Yii::$app->db->createCommand($sql)->execute();
            ...
            // var_dump($model);
            // die();
              $logmodel->createUpdateLogRule("FileUploadCustomer: ","",$fileName,"INSERT");
