...
function saveInteraction($form, $reload_url, case_id) {
 //alert("entered saveInteraction1");
  $btn = $form.find(':submit');
  $btn.button('loading');
  console.log("in save interaction");
  /*
  var formElement = document.querySelector("form");
  var formData = new FormData(formElement);
...
function saveInteraction2($form, $reload_url, case_id) {
  //alert("entered saveInteraction2");
  $btn = $form.find(':submit');
  $btn.button('loading');
...
function reloadEevents($url) {
  $('#create-customerevents-modal-button').button('loading');
  Loading('customerevents-widget', false);
  $('#customerevents-view').load($url);
}
