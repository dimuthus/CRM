    ...
    class CasesController extends Controller
    {
        ...
        public function actionIndex()
        {
           ...
           $searchModel =new CasesSearch();
			...
			->select(['customer_cases.id','customer_cases.case_id','customer_cases.created_datetime','case_escalation.escalated_to','customer_cases.case_status','customer_cases.created_by'])
            //->where('user.id != 1 AND customer_cases.case_status NOT IN (2,4)')
			//->andWhere(' customer_cases.case_status NOT IN (2)')
	        ->joinWith('caseEscalation')
            ->joinWith('customer')
            //->joinWith('caseType')
            ->joinWith('caseStatus')
          ...
          //  die('dddddd');
            if(!Yii::$app->user->can('View Service Reuqests Created by All Agents')){
                $query->andFilterWhere(['and',['customer_cases.created_by'=> Yii::$app->user->identity->id],['case_escalation.escalated_to'=> Yii::$app->user->identity->id]]);
			//->orFilterWhere(['customer_cases.escalated_to'=> Yii::$app->user->identity->id]);
			}
			else {
				$query->andFilterWhere(['case_escalation.escalated_to'=> Yii::$app->user->identity->id]);
      ...
      $logmodel->createViewLog("Cases",$data);
        ...
        {   
			$caseModel=$this->findModel($id);
			$logmodel= new Logadm();
            $data = array('case_id' => $caseModel['case_id']);
            $data=json_encode($data);
            ...
            return $this->renderAjax('view', [
                                     'model' => $caseModel,'campaign_id'=>$campaign_id
                                     ]);
        }
            ...
            // && Yii::$app->request->isAjax
             if ($model->load(Yii::$app->request->post()) && Yii::$app->request->isAjax) {
               ...
               // die("escalated_to");
				...
				$c_username=Yii::$app->user->identity->username;
				$cnote=Yii::$app->request->post('CustomerCases')['case_note'] ;
				$model->case_note="Created By ".$c_username." on ".$mystr.">>>".$cnote;
				//die($model->case_note);
                ...
                if($model->save()) {
					...
					$case_note=$model->case_note;
					if ($model->escalated_to){
						 $escalated_to=$model->escalated_to;
						 $model->created_by = Yii::$app->user->identity->id;

				   $this->escalationLog($case_id,$escalated_to,$model->created_by,$case_note);
					}
                    $model = new InboundInteraction();
                    $model->created_by = Yii::$app->user->identity->id;
                    $model->case_tbl_id = $case_id;
                    //$model->case_status = $case_status;
                    Yii::$app->session->setFlash('success', "Case sucessfully saved."); 
        ...
        public function actionUpdate($id)
        {
            ...
            if ($model->load(Yii::$app->request->post()) && Yii::$app->request->isAjax) {
				...
				$c_username=Yii::$app->user->identity->username;
				$case_note_update=Yii::$app->request->post('CustomerCases')['case_note_update'] ;
				$model->case_note=$previousNote."<br>"."Created By ".$c_username." on ".$mystr.">>>".$case_note_update;
                if($model->save()) {
                            $data=Yii::$app->request->post();
             // var_dump($dataUpdate);
   ...
   public function actionPopulateoc1outbound()
    {
		...
		public function escalationLog($case_id, $escalated_to,$escalated_by, $escalation_remarks){
			
			if ($escalated_to==91){
					$query = new Query;
					$query->select('id')
					->from('user')
					->where("role_id = 'BOC_ES_TEAM' and id != ".$escalated_to);
					$rows = $query->all();
					foreach ($rows as $user) {
						$escalated_to=$user['id'];
						$sql="INSERT INTO `case_escalation` (case_id,escalated_to,escalated_by,escalation_remarks) VALUES ($case_id,$escalated_to,$escalated_by,'$escalation_remarks')";
