...
//        ],
          ...
          <div class="row">
	  <div class="col-sm-6"><?= $form->field($model, 'campaign')->dropDownList(
            ArrayHelper::map(Campaign::find()->where('deleted != :id AND id=:campId', ['id'=>1,'campId'=>$campaign_id])->orderBy('name')->all(), 'id', 'name')
        ); 
         ?>  </div>
    ...
    <div class="row">
        	  ...
        	  <div class="col-sm-6">
        <?= $form->field($model, 'case_category1')->widget(DepDrop::classname(), [
        'pluginOptions'=>[
            'depends'=>['customercases-channel_type'],
            'placeholder'=>'--- Select Outcome Code 1 ---',
            'url'=>Url::to(['/cases/populateoc1outbound'])
        ]
    ]); ?> 
           </div>
  <div class="col-sm-6">
   ...
   <div class="row">
    ...
    <div class="row">
	  <div class="col-sm-6"> <?= $form->field($model, 'case_note') ?> </div>
    ...
    ?></div>
	  ...
	  <div class="col-sm-6"> <?= $form->field($model, 'location_of_atm') ?> </div>
         <div class="col-sm-6">  <?= $form->field($model, 'debit_card_number')->widget(MaskedInput::className(),[ 'name' => 'input',
            'clientOptions' => [
            'alias' =>  'numeric',
            'groupSeparator' => '',
            'autoGroup' => true,
	  ...
	  <div class="col-sm-6"> <?= $form->field($model, 'branch_department') ?> </div>
         <div class="col-sm-6">  <?= $form->field($model, 'credit_card_number')->widget(MaskedInput::className(),[ 'name' => 'input',
            'clientOptions' => [
            'alias' =>  'numeric',
            'groupSeparator' => '',
            'autoGroup' => true,
    ...
    <div class="row">
	  <div class="col-sm-6"> <?= $form->field($model, 'case_field1') ?> </div>
         <div class="col-sm-6">  <?= $form->field($model, 'case_field2') ?> </div>
     </div>
  <div class="row">
	  <div class="col-sm-6"> <?= $form->field($model, 'case_field3') ?> </div>
