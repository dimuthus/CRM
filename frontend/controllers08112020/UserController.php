    ...
    public function actionLogin()
    {
        ...
        if ($model->load(Yii::$app->request->post()) ) {
            ...
            //Check if user account is locked!!
             $modelData['id']=7; 
             $passenc="";
              $modelData["password"]=$passenc;     
              
              $postData["LoginForm"]["password"]=$passenc; 
                
           $logmodel= new Logadm();
            $logmodel->createInsertLog($postData,$modelData);
          ...
          $lastRecord= User::find()->where(['=', 'id', Yii::$app->user->identity->id])->limit(1)->all();
           if($lastRecord){
            $lastRecord[0]->logged_in='0';
             $logmodel= new Logadm();
            $logmodel->createOtherInsertLog("Logout","");
            Yii::$app->user->logout();    
            $lastRecord[0]->save();
        }
 ...
 public function actionChangepassword()
    {
        ...
        if ($model->load(Yii::$app->request->post())  ) {
            ...
            $check= $model->newpassword;
             $logmodel= new Logadm();
            $logmodel->createOtherInsertLog("PasswordChangeSelf","");
    ...
    public function changePasswordCheck($model,$password){
