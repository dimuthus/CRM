...
<div class="complaint-product-index">
    ...
    <?php $form = ActiveForm::begin(['id' => 'dropdown-complaint-packsize-form']); ?>
    ...
    <?= $form->field($model, 'subband_id')->widget(DepDrop::classname(), [
        'pluginOptions'=>[
            'depends'=>['complaintpacksize-brand_id'],
    ...
    <?= $form->field($model, 'product_id')->widget(DepDrop::classname(), [
        'pluginOptions'=>[
            'depends'=>['complaintpacksize-subband_id'],
            'placeholder'=>'--- Select a Product ---',
            'url'=>Url::to(['/complaint/populateproductddl'])
        ]
    ])->label(false); ?>

    <div id="complaint-packsize-box" style="display:none">
        ...
        <div id="complaint-packsize-widget">
            <?= $this->render(Url::to('complaint_packsize_list'), [
...
<?php
    $this->registerJs("
        ...
        $('#complaintpacksize-product_id').change(function (e) {
            if(!$(this).val()) {
                $('#complaint-packsize-box').hide();
                return;
            }
            $('#complaint-packsize-box').show();
            Loading('complaint-packsize-box',true);
            $.ajax({
              url: $('#dropdown-complaint-packsize-form').attr('action'),
              type: 'get',
              data: {'refresh-widget':true,'product_id':$(this).val()},
              success: function(response) {
                $('#complaint-packsize-widget').html(response); 
                Loading('complaint-packsize-box',true);
              }
            });
        });

        $('#dropdown-complaint-packsize-form').submit(function (e) {
            e.preventDefault();
            ...
            $.ajax({
                ...
                success: function(response) {
                    if(!response.hasError) {
                        $('#complaint-packsize-widget').html(response.body);
                        btn.button('reset');
