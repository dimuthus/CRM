...
<div class="request-form">
    ...
    <?= $form->field($model, 'detail_type')->dropDownList(
            ArrayHelper::map(RequestDetailType::find()->where('deleted != :id and type_id=:type_id', ['id'=>1,'type_id'=>$model->service_request_type])->orderBy('name')->all(), 'id', 'name'),
            ['prompt'=>'-----', 'disabled'=>'disabled']
        ); 
    ?>

    <?= $form->field($model, 'additional_detail')->dropDownList(
            ArrayHelper::map(RequestAdditionalType::find()->where('deleted != :id and detail_type_id=:detail_id', ['id'=>1,'detail_id'=>$model->detail_type])->orderBy('name')->all(), 'id', 'name'),
            ['prompt'=>'-----', 'disabled'=>'disabled']
        ); 
    ?>

    <?= $form->field($model, 'supplemental_detail')->dropDownList(
            ArrayHelper::map(RequestSupplementalType::find()->where('deleted != :id and additional_type_id=:additional_type', ['id'=>1,'additional_type'=>$model->additional_detail])->orderBy('name')->all(), 'id', 'name'),
            ['prompt'=>'-----', 'disabled'=>'disabled']
        ); 
    ?>
    ...
    <div class="form-group">
        ...
        <div class="col-sm-8">
           <?= Html::submitButton('Save Changes', ['class' => 'btn btn-sm btn-primary']) ?>
        </div>
    </div>
