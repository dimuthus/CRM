        ...
        $('.ui-dialog-title').attr('style','font-size:12px;font-weight:bold;');
				...
				buttons: {
							...
							$.post('../cases/finance/saveClaimApproval.php',{'lmChecked':rSelection,'lmComment':lmComment,'hdClaimId':claimId,'stage':stage},function(res){
								...
								approvalCount=0;
								//alert(res);
								$('#divResult_'+claimId).html('This item is:<h style="color:red;">not included</h>  Remarks:<h style="color:red;">'+lmComment+'<h>');
			...
			position: "center" ,
						  ...
						  buttons: {
							"Save": function() {
								...
								var txtTotalE=$('#txtTotalE').val();
								var rSelection=$("input[name='expBillable']:checked").val();
								var clientName=$('#clientName').val();
								var expReceiptNo=$('#expReceiptNo').val();

								
								//alert(rSelection);
								//alert(expCurrency+expRateB+txtTotalE);
								//return false;
								//td_totalVal_419
							  ...
							  $.ajax({
								  url:"../cases/finance/saveClaim.php",
									...
									success:function(response){
										...
										if (rSelection==1)
										 $('#td_billable_'+claimId).html("Yes");
										else
										 $('#td_billable_'+claimId).html("No");
										 $('#td_clientname_'+claimId).html(clientName);
										 $('#td_taxinvoice_'+claimId).html(expReceiptNo);
...
switch ($option){
	...
	$GTOTAL="0.00";
		...
		foreach ($claimRes as $claimInfo){
			echo "<tr><td>";
			echo createClaimViewOPS($claimInfo,$option,$displayCurrency);
			echo "</td>";
        ...
        <tr><td width='55%'></td><td style='font-size:12px;font-weight:bold; width: 20%;'>Less Advance Taken</td><td style='font-size:12px;font-weight:bold; width: 10%;' id='td_LessAdvance' >".$displayCurrency." ".$LESS_ADVANCES."</td><td width='10%'></td></tr>
		...
		<tr><td width='55%'></td><td style='font-size:12px;font-weight:bold; width: 10%;'>Grand Total</td><td style='font-size:12px;font-weight:bold; width: 10%;' id='td_OverallTot'>".$displayCurrency." ".$overAllTotal."</td><td width='10%'><input type='hidden' value=".$overAllTotal." id='txtGTotal'/></td></tr></table>";
		}
		break;
	case 'PM':
	case 'HOD':
	...
	$total=0.0;
		...
		foreach ($claimRes as $claimInfo){
			...
			echo createClaimView($claimInfo,$option,$displayCurrency);
			echo "</td>";
...
function createClaimViewOPS($claimInfo,$option,$displayCurrency){
...
function createClaimView($claimInfo,$option,$displayCurrency){
	$fileName=isset($claimInfo['FILE_NAME'])?$fileName=$claimInfo['FILE_NAME']:'';
	$AMOUNT=number_format((float)$claimInfo['AMOUNT'], 2, '.', '');
	$RATE=number_format((float)$claimInfo['RATE'], 4, '.', '');
	...
	if ($claimInfo['CURRENCY']!='MYR'){
		...
		$partialViewCurrencyRate="<td class='tg-yw4l'></td><td ></td><td class='tg-txt' id='td_rateVal_".$claimInfo['CLIM_ID']."' ></td>";
	}
	$efUpdateBtnDisplay="";
	$chkEfRadio="";
	$RadioStrYes="checked='true'";
	...
	$approvalOption="<div><div style='float:right;background-color: #134480;'><table bgcolor='#00FF00'><tr><td>Include this item in the claim?</td><td><label class='radio-inline'><input name='lmAction_".$claimInfo['CLIM_ID']."' value='1' type='radio' $RadioStrYes onclick='updateClaim(".$claimInfo['CLIM_ID'].",".$claimInfo['APP_NUMBER'].",".$claimInfo['CLIM_SEQ'].",\"$option\")'>Yes</label><label class='radio-inline'><input name='lmAction_".$claimInfo['CLIM_ID']."' value='0' type='radio' $RadioStrNo  onclick='updateClaim(".$claimInfo['CLIM_ID'].",".$claimInfo['APP_NUMBER'].",".$claimInfo['CLIM_SEQ'].",\"$option\")'>No</label></div>
	</td></tr></table></div></br>
	<div style='display:none' id='divPMcomments_".$claimInfo['CLIM_ID']."'><textarea style='width:100%;font-family: Arial, sans-serif;font-size: 12px;' id='PMremarks_".$claimInfo['CLIM_ID']."' ></textarea><span style='color:red;' id='error_".$claimInfo['CLIM_ID']."' ></span></div>";
	
	$approvalResult="<div id='divResult_".$claimInfo['CLIM_ID']."' style='background-color:kahaki'></div>";
	...
	<table id='".$claimInfo['CLIM_ID']."' class='tg' width='100%' cellspacing='0' cellpadding='0'>
	  <tr>
		...
		$efUpdateBtnDisplay $approvalOption
          </th>
	  </tr>
	  <tr>
		<td class='tg-yw4l'>Expense Type<br></td>
    ...
    <td>:</td>
		...
		<td class='tg-yw4l'>Billable to Client</td>
    <td>:</td>
		<td class='tg-txt' id='td_billable_".$claimInfo['CLIM_ID']."'>".$claimInfo['BILLABLE']."<br></td>$partialViewCurrencyRate
    ...
    <td>:</td>
		...
		<td class='tg-yw4l'>Tax invoice<br></td>
    <td>:</td>
		<td class='tg-txt' id='td_taxinvoice_".$claimInfo['CLIM_ID']."'>".$claimInfo['RECEIPT_NO']."</td>
		<td class='tg-yw4l'>Total Expenses</td>
    <td>:</td>
		<td class='tg-txt' id='td_totalVal_".$claimInfo['CLIM_ID']."'><span>".$displayCurrency."</span> ".$TOTAL_AMOUNT."</td>
	  </tr>
    ...
    <td>:</td>
		<td class='tg-txt' id='td_clientname_".$claimInfo['CLIM_ID']."' >".$claimInfo['CLIENT_NAME']."</td><td class='tg-yw4l'><br></td><td></td><td class='tg-txt' id='td_totalVal00_".$claimInfo['CLIM_ID']."'><br></td>
	  </tr></tr><tr><td colspan='10'>$approvalResult</td></tr><tr><td colspan='10'></td>";
	 //die($option);
