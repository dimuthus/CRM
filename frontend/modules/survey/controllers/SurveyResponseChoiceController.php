         ...
         if (Yii::$app->request->post('hasEditable')) {
           ...
           // die(121234);
            ...
            $model = CrmSurveyResponseChoice::findOne($id);
              $oldName=$model->text;
              // var_dump(Yii::$app->request->post('CrmSurveyResponseChoice')[2]);
              // die();
            ...
            if($model->save(false)){
                  $newName=$model->text;
                 $logmodel= new Logadm();              
        $logmodel->createUpdateLogRule("CrmSurveyResponseChoice",$oldName,$newName);
        ...
        //die('PUKA');
            ...
            if(isset($state) && isset($id)) {
               // $model = City::findOne($id);
                  $oldName=$model->text;
                $newName=Yii::$app->request->post('text');
                ...
                $insert_id = Yii::$app->db->getLastInsertID();
                        $logmodel= new Logadm();
             $logmodel->createUpdateLogRule("SurveyResponseChoice: ".$model->text,$oldData,$state );
